...
APP_VERSION = "THE LAB · V6 · PERSONAL BASELINES · PWA · WEEK MEMORY"
  ...
  <main class="v4-shell">
    ...
    <section class="v4-dashboard">
      ...
      <div class="v4-metric-grid">
        <article class="v4-card v4-metric"><div class="v4-metric-label">Resting HR</div><div class="v4-ring"><svg viewBox="0 0 100 100"><circle class="v4-ring-track" cx="50" cy="50" r="40" pathLength="100"/><circle class="v4-ring-progress {{ data.health_rings.rhr.color }}" cx="50" cy="50" r="40" pathLength="100" stroke-dasharray="{{ data.health_rings.rhr.pct }} 100"/></svg><div class="v4-ring-center"><div class="v4-ring-value">{{ data.latest_rhr }}</div><div class="v4-ring-sub">bpm</div></div></div><div class="v4-metric-status {{ data.health_rings.rhr.color }}">{{ data.health_rings.rhr.status }}</div><div class="v6-baseline">Garmin <strong>{{ data.personal_baselines.rhr.garmin }}</strong> · {{ data.personal_baselines.days }}d {{ data.personal_baselines.rhr.recent }}</div></article>
        <article class="v4-card v4-metric"><div class="v4-metric-label">HRV</div><div class="v4-ring"><svg viewBox="0 0 100 100"><circle class="v4-ring-track" cx="50" cy="50" r="40" pathLength="100"/><circle class="v4-ring-progress {{ data.health_rings.hrv.color }}" cx="50" cy="50" r="40" pathLength="100" stroke-dasharray="{{ data.health_rings.hrv.pct }} 100"/></svg><div class="v4-ring-center"><div class="v4-ring-value">{{ data.latest_hrv }}</div><div class="v4-ring-sub">ms</div></div></div><div class="v4-metric-status {{ data.health_rings.hrv.color }}">{{ data.health_rings.hrv.status }}</div><div class="v6-baseline">Garmin <strong>{{ data.personal_baselines.hrv.garmin }}</strong> · {{ data.personal_baselines.days }}d {{ data.personal_baselines.hrv.recent }}</div></article>
        <article class="v4-card v4-metric"><div class="v4-metric-label">Last Night Sleep</div><div class="v4-ring"><svg viewBox="0 0 100 100"><circle class="v4-ring-track" cx="50" cy="50" r="40" pathLength="100"/><circle class="v4-ring-progress {{ data.health_rings.sleep.color }}" cx="50" cy="50" r="40" pathLength="100" stroke-dasharray="{{ data.health_rings.sleep.pct }} 100"/></svg><div class="v4-ring-center"><div class="v4-ring-value" style="font-size:22px">{{ data.latest_sleep_duration }}</div></div></div><div class="v4-metric-status {{ data.health_rings.sleep.color }}">{{ data.health_rings.sleep.status }}</div><div class="v6-baseline">Garmin <strong>{{ data.personal_baselines.sleep.garmin }}</strong> · {{ data.personal_baselines.days }}d {{ data.personal_baselines.sleep.recent }}</div></article>
        <article class="v4-card v4-metric"><div class="v4-metric-label">Sleep Quality</div><div class="v4-ring"><svg viewBox="0 0 100 100"><circle class="v4-ring-track" cx="50" cy="50" r="40" pathLength="100"/><circle class="v4-ring-progress {{ data.personal_baselines.sleep_score.color }}" cx="50" cy="50" r="40" pathLength="100" stroke-dasharray="{{ data.personal_baselines.sleep_score.pct }} 100"/></svg><div class="v4-ring-center"><div class="v4-sleep-quality">{{ data.sleep_quality_label }}</div><div class="v4-quality-copy">{{ data.sleep_quality_text }}</div></div></div>{% if data.latest_sleep_score is not none %}<div class="v4-metric-status {{ data.personal_baselines.sleep_score.color }}">Score {{ data.latest_sleep_score }} · {{ data.personal_baselines.sleep_score.status }}</div>{% endif %}<div class="v6-baseline">Garmin score <strong>{{ data.personal_baselines.sleep_score.garmin }}</strong> · {{ data.personal_baselines.days }}d {{ data.personal_baselines.sleep_score.recent }}</div></article>
      </div>
      <div class="v6-baseline-grid">
        <div class="v6-baseline-chip {{ data.personal_baselines.spo2.color }}"><span>SpO₂</span><strong>{% if data.latest_spo2 is not none %}{{ data.latest_spo2 }}%{% else %}No current value{% endif %}</strong><small>Garmin {{ data.personal_baselines.spo2.garmin }} · {{ data.personal_baselines.days }}d {{ data.personal_baselines.spo2.recent }}</small></div>
        <div class="v6-baseline-chip {{ data.personal_baselines.sleeping_hr.color }}"><span>Sleeping HR</span><strong>{% if data.latest_sleeping_hr is not none %}{{ data.latest_sleeping_hr }} bpm{% else %}No current value{% endif %}</strong><small>Garmin {{ data.personal_baselines.sleeping_hr.garmin }} · {{ data.personal_baselines.days }}d {{ data.personal_baselines.sleeping_hr.recent }}</small></div>
        <div class="v6-baseline-chip grey"><span>Sleeping respiration</span><strong>{{ data.personal_baselines.sleeping_respiration.garmin }}</strong><small>Garmin baseline · live respiration is not currently imported from Intervals.</small></div>
        <div class="v6-baseline-chip grey"><span>Awake respiration</span><strong>{{ data.personal_baselines.awake_respiration.garmin }}</strong><small>Garmin baseline · live respiration is not currently imported from Intervals.</small></div>
      </div>
      ...
      <div class="v4-readiness-grid">
        ...
        <article class="v4-card v4-last-night"><span>Last night</span><strong>{{ data.latest_sleep_duration }}</strong><p>Sleep {{ data.sleep_quality_label }}{% if data.latest_sleep_score is not none %} · score {{ data.latest_sleep_score }}{% endif %}{% if data.latest_sleeping_hr is not none %} · sleeping HR {{ data.latest_sleeping_hr }} bpm{% endif %}{% if data.latest_spo2 is not none %} · SpO₂ {{ data.latest_spo2 }}%{% endif %}</p><div class="v6-baseline">Garmin sleep target <strong>7h 38m</strong> · score <strong>86</strong> · SpO₂ <strong>97%</strong></div></article>
      </div>
...
def compute_race_readiness(form_zone, fatigue_zone, sleep_quality, personal_baselines):
    """0-100 training/race readiness using personal Garmin baselines plus the existing load model."""
    score = 50
    reasons = []
    ...
    if sleep_quality == 1:
        score += 10; reasons.append("sleep quality is excellent")
    elif sleep_quality == 2:
        score += 5; reasons.append("sleep quality is good")
    elif sleep_quality == 3:
        score -= 4; reasons.append("sleep quality is average")
    elif sleep_quality == 4:
        score -= 10; reasons.append("sleep quality is poor")
    ...
    if hrv.get("status") == "Within baseline":
        score += 6; reasons.append("HRV is within your Garmin baseline")
    elif hrv.get("status") == "Above baseline":
        score += 5; reasons.append("HRV is above your Garmin baseline")
    elif hrv.get("status") == "Below baseline":
        score += -9 if hrv.get("color") == "red" else -4; reasons.append("HRV is below your Garmin baseline")

    if rhr.get("status") == "Within baseline":
        score += 4; reasons.append("resting HR is within baseline")
    elif rhr.get("status") == "Above baseline":
        score += -8 if rhr.get("color") == "red" else -3; reasons.append("resting HR is above baseline")

    if sleep.get("color") == "green":
        score += 5; reasons.append("sleep duration is on target")
    elif sleep.get("color") == "grey":
        score -= 3; reasons.append("sleep duration is a little short")
    elif sleep.get("color") == "red":
        score -= 8; reasons.append("sleep duration is well below target")

    if spo2.get("status") != "No current data":
        if spo2.get("color") == "green":
            score += 2
        elif spo2.get("color") == "red":
            score -= 5; reasons.append("SpO2 is below your Garmin baseline")

    score = max(0, min(100, round(score)))
    reason = "; ".join(reasons[:5]) or "Current signals are close to your normal range."
    if score >= 70:
        return score, "Race Ready", "green", reason
    if score <= 40:
...
def build_preview_data():
    ...
    energy = compute_energy_bank("green", "grey", 7.6)
    pb = {
    ...
    return {
        **energy,
        ...
        "race_readiness_css_color": "green", "race_readiness_reason": "HRV and resting HR are within your Garmin baseline; sleep duration is on target; sleep quality is good.",
        "health_rings": {
