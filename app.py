...
APP_VERSION = "THE LAB · FONT FIX v3.6 · WEEK MEMORY"
...
# ---------------------------------------------------------------------------
BASE_CSS = """
@import url('https://fonts.googleapis.com/css2?family=Amatic+SC:wght@400;700&family=Manrope:wght@400;500;600;700&family=Parkinsans:wght@400;500;600;700&display=swap');
...
/* LAB Performance v2 ----------------------------------------------------- */
.performance-kicker {
  position:fixed; z-index:20; right:16px; bottom:10px;
...
.performance-kicker::before { display:none !important; }
...
.sleep-q { font-family:'Parkinsans',sans-serif; font-size:38px; line-height:1; margin:13px 0 5px; color:var(--sleep-accent); }
...
.readiness-value { font-family:'Parkinsans',sans-serif; font-size:28px; margin-top:3px; }
...
.polar-center { position:relative; z-index:1; text-align:center; font-family:'Parkinsans',sans-serif; font-size:25px; line-height:1; }
.polar-center small { display:block; font-family:'Manrope',sans-serif; font-size:8px; color:var(--grey-zone); letter-spacing:.10em; margin-top:5px; }
...
/* THE LAB · COACH CLOCK v3.0 ------------------------------------------- */
.coach-clock-preview {
  margin: 12px 0 4px;
  padding: 10px 12px;
  border: 1px solid rgba(69,215,232,.25);
  border-radius: 10px;
...
.coach-clock-preview .clock-mini-label { color: var(--cyan); font-size: 9px; letter-spacing: .14em; }
.coach-clock-preview strong { font-family:'Parkinsans',sans-serif; font-size: 23px; text-align:center; }
.coach-clock-preview small { color: var(--grey-zone); font-size: 9px; text-transform:uppercase; letter-spacing:.08em; }
...
.coach-clock-card {
  ...
  margin-bottom: 18px;
  padding: 22px;
  border: 1px solid rgba(69,215,232,.20);
  border-radius: 18px;
  background:
    radial-gradient(circle at 12% 0%, rgba(69,215,232,.11), transparent 38%),
    linear-gradient(180deg, rgba(255,255,255,.035), rgba(255,255,255,.012)),
    var(--panel);
...
.clock-face {
  ...
  margin:auto;
  position:relative;
  background: conic-gradient(var(--cyan) calc(var(--clock-pct) * 1%), rgba(255,255,255,.08) 0);
  box-shadow: 0 0 35px rgba(69,215,232,.10);
}
.clock-face:after {
  content:"";
  position:absolute;
  inset: 9px;
  border-radius:50%;
  background: radial-gradient(circle at 50% 18%, rgba(69,215,232,.08), transparent 35%), #0d0f10;
  border: 1px solid rgba(255,255,255,.08);
}
.clock-face-inner { position:relative; z-index:1; text-align:center; }
.clock-live { font-family:'Parkinsans',sans-serif; font-size:47px; line-height:.9; letter-spacing:.03em; }
.clock-zone { margin-top:8px; color:var(--cyan); font-size:10px; letter-spacing:.16em; text-transform:uppercase; }
.clock-date { margin-top:5px; color:var(--grey-zone); font-size:10px; }
.clock-copy h3 { margin:0; font-family:'Parkinsans',sans-serif; font-size:31px; letter-spacing:.03em; font-weight:400; }
.clock-copy > p { margin:7px 0 14px; color:var(--grey-zone); line-height:1.55; font-size:13px; }
.slot-status-grid { display:grid; grid-template-columns:1fr 1fr; gap:10px; }
.slot-status {
...
.slot-status .slot-label { font-size:9px; letter-spacing:.14em; color:var(--grey-zone); text-transform:uppercase; }
.slot-status strong { display:block; margin-top:4px; font-family:'Parkinsans',sans-serif; font-size:22px; font-weight:400; }
.slot-status.done strong { color:var(--green); }
.slot-status.open strong { color:var(--cyan); }
.slot-status.upcoming strong { color:var(--orange); }
.slot-status.missed strong { color:var(--grey-zone); }
.next-slot-ribbon {
  margin-top:12px;
  padding:12px 14px;
  border-radius:12px;
  border:1px solid rgba(255,159,28,.24);
  background:linear-gradient(90deg, rgba(255,159,28,.10), rgba(255,159,28,.02));
  display:flex;
  align-items:center;
  justify-content:space-between;
  gap:12px;
}
.next-slot-ribbon span { color:var(--grey-zone); font-size:10px; text-transform:uppercase; letter-spacing:.12em; }
.next-slot-ribbon strong { font-family:'Parkinsans',sans-serif; font-size:20px; font-weight:400; color:var(--orange); text-align:right; }
.today-activities { margin-top:10px; font-size:11px; color:var(--grey-zone); line-height:1.45; }

.readiness-panel.readiness-gauge-panel { display:grid; grid-template-columns:112px 1fr; align-items:center; gap:16px; }
.readiness-ring {
  --readiness: 50;
  --readiness-color: var(--grey-zone);
  width:104px;
  aspect-ratio:1;
  border-radius:50%;
  display:grid;
  place-items:center;
  background:conic-gradient(var(--readiness-color) calc(var(--readiness) * 1%), rgba(255,255,255,.08) 0);
  position:relative;
  box-shadow:0 0 26px color-mix(in srgb, var(--readiness-color) 16%, transparent);
}
.readiness-ring:after { content:""; position:absolute; inset:8px; border-radius:50%; background:#101010; }
.readiness-ring-inner { position:relative; z-index:1; text-align:center; }
.readiness-ring-score { font-family:'Parkinsans',sans-serif; font-size:34px; line-height:.9; }
.readiness-ring-sub { font-size:8px; color:var(--grey-zone); margin-top:4px; }
...
.coach-call-box {
  ...
  border: 1px solid rgba(255,59,79,.28);
  background:
    radial-gradient(circle at 0 0, rgba(255,59,79,.13), transparent 34%),
    linear-gradient(180deg, rgba(255,255,255,.03), rgba(255,255,255,.01)),
    var(--panel);
  box-shadow: 0 16px 40px rgba(0,0,0,.26);
}
.coach-call-kicker { color:var(--red); font-size:10px; letter-spacing:.18em; text-transform:uppercase; font-weight:700; }
.coach-call-title { font-family:'Parkinsans',sans-serif; font-size:30px; margin:4px 0 7px; }
.coach-call-box p { margin:0; color:#dedede; line-height:1.62; font-size:14px; }


/* THE LAB · WEEK MEMORY v3.3 ------------------------------------------- */
.week-memory-bar {
  display:flex;
  align-items:center;
  gap:14px;
  ...
  border:1px solid rgba(255,255,255,.10);
  border-radius:16px;
  background:linear-gradient(135deg,rgba(69,215,232,.07),rgba(255,255,255,.025));
}
.week-memory-label {
...
.week-memory-items { display:flex; gap:8px; flex-wrap:wrap; flex:1; }
.week-memory-chip {
  display:inline-flex;
  align-items:center;
  gap:7px;
...
.week-memory-chip strong { font-family:'Parkinsans',sans-serif; font-weight:650; color:#fff; }
.week-memory-chip.hard { border-color:rgba(255,159,28,.35); background:rgba(255,159,28,.08); }
.week-memory-chip.easy { border-color:rgba(37,212,122,.24); }
.week-memory-empty { font-size:12px; color:var(--grey-zone); }
.week-memory-note { width:100%; font-size:11px; color:var(--grey-zone); line-height:1.45; }
@media (max-width:650px) {
  .week-memory-bar { align-items:flex-start; padding:13px; }
  .week-memory-label { width:100%; }
  .week-memory-chip { font-size:10px; }
}

.next-sessions-section { margin: 20px 0 24px; }
.next-sessions-head { display:flex; justify-content:space-between; align-items:flex-end; gap:12px; margin-bottom:12px; }
.next-sessions-head h2 { margin:0; }
.next-sessions-head p { margin:0; font-size:11px; color:var(--grey-zone); text-align:right; }
.next-sessions-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; }
.session-card {
  --session-accent: var(--cyan);
  position:relative;
  overflow:hidden;
...
.session-card:before { content:""; position:absolute; left:0; top:0; bottom:0; width:3px; background:var(--session-accent); box-shadow:0 0 14px var(--session-accent); }
.session-card.intensity-recovery { --session-accent: var(--green); }
.session-card.intensity-endurance { --session-accent: var(--cyan); }
.session-card.intensity-tempo { --session-accent: var(--orange); }
.session-card.intensity-threshold { --session-accent: var(--magenta); }
.session-card.intensity-vo2 { --session-accent: var(--red); }
.session-slot { font-size:9px; color:var(--session-accent); letter-spacing:.14em; text-transform:uppercase; font-weight:700; }
.session-title { font-family:'Parkinsans',sans-serif; font-size:25px; line-height:1.05; margin:7px 0 9px; }
.session-meta { display:flex; gap:7px; flex-wrap:wrap; margin-bottom:12px; }
.session-pill { font-size:9px; padding:4px 7px; border-radius:999px; border:1px solid rgba(255,255,255,.13); color:var(--grey-zone); }
.session-main { font-size:12px; line-height:1.55; color:#e8e8e8; padding:11px 0; border-top:1px solid rgba(255,255,255,.07); border-bottom:1px solid rgba(255,255,255,.07); white-space:pre-line; }
.session-why { margin-top:11px; font-size:11px; color:var(--grey-zone); line-height:1.5; }
.session-why strong { color:var(--white); }

.metric-widget-row { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; margin-bottom:14px; }
.metric-widget { border:1px solid rgba(255,255,255,.11); border-radius:14px; padding:15px; background:rgba(255,255,255,.02); }
.metric-widget .stat-label { margin-bottom:5px; }
.metric-widget .stat-value { font-size:30px; }
.metric-meter { height:5px; border-radius:999px; background:rgba(255,255,255,.08); overflow:hidden; margin-top:10px; }
.metric-meter > i { display:block; height:100%; border-radius:inherit; background:linear-gradient(90deg,var(--cyan),var(--green)); }

@media (max-width: 980px) {
  .coach-clock-card { grid-template-columns:170px 1fr; }
  .clock-face { width:155px; }
  .next-sessions-grid { grid-template-columns:1fr; }
}
@media (max-width: 650px) {
  .coach-clock-preview { grid-template-columns:1fr auto; }
  .coach-clock-preview small { grid-column:1 / -1; }
  .coach-clock-card { grid-template-columns:1fr; gap:16px; padding:17px 15px; }
  .clock-face { width:160px; }
  .clock-live { font-size:42px; }
  .slot-status-grid { grid-template-columns:1fr 1fr; }
  .readiness-panel.readiness-gauge-panel { grid-template-columns:96px 1fr; }
  .readiness-ring { width:90px; }
  .next-sessions-head { align-items:flex-start; flex-direction:column; }
  .next-sessions-head p { text-align:left; }
  .metric-widget-row { grid-template-columns:1fr; }
}


/* THE LAB · VISUAL POLISH v3.2 ---------------------------------------- */
:root {
  --font-main: 'Manrope', sans-serif;
  --font-display: 'Parkinsans', sans-serif;
  --font-hand: 'Amatic SC', cursive;
...
.display, .page-title, .section-title, .energy-bank-label, .readiness-value,
.clock-live, .clock-copy h3, .slot-status strong, .next-slot-ribbon strong,
.readiness-ring-score, .coach-call-title, .session-title, .sleep-q, .polar-center,
.prose-card h3, .recommendation-box h3, .training-tips-box h3, .wearable-stat strong {
  font-family: var(--font-display) !important;
  letter-spacing: -0.015em;
}
.eyebrow, .coach-call-kicker, .clock-mini-label, .micro-label, .brand-script {
  font-family: var(--font-hand) !important;
  letter-spacing: .04em;
  text-transform: none !important;
  font-weight: 700;
}
.top-bar .eyebrow { font-size: 28px; line-height: 1; }
.logout-link {
  display:inline-flex; align-items:center; gap:8px; padding:9px 14px;
  border:1px solid rgba(255,255,255,.22); border-radius:999px; text-decoration:none;
  background:rgba(255,255,255,.04); color:var(--white); font-family:var(--font-main);
  font-size:13px; font-weight:700; transition:.2s ease; box-shadow:0 6px 18px rgba(0,0,0,.18);
}
.logout-link::before { content:'↗'; font-size:13px; transform:rotate(90deg); opacity:.85; }
.logout-link:hover { border-color:var(--red); background:rgba(255,59,79,.10); transform:translateY(-1px); }

/* Coach Clock: clearer donut + stronger hierarchy */
.coach-clock-card { border-radius:24px; padding:24px; background:linear-gradient(145deg, rgba(69,215,232,.08), rgba(255,255,255,.02) 40%), var(--panel); }
.coach-clock-layout { align-items:center; gap:26px; }
.coach-clock-visual { width:190px; height:190px; min-width:190px; border-radius:50%; position:relative; display:grid; place-items:center;
  background:conic-gradient(from -90deg, rgba(69,215,232,.95) 0 33%, rgba(142,148,157,.28) 33% 70%, rgba(255,159,28,.95) 70% 100%);
  box-shadow:0 0 0 1px rgba(255,255,255,.06), 0 18px 40px rgba(0,0,0,.28), 0 0 36px rgba(69,215,232,.08);
}
.coach-clock-visual::before { content:''; position:absolute; inset:18px; border-radius:50%; background:#151515; box-shadow:inset 0 0 0 1px rgba(255,255,255,.06); }
.coach-clock-face { position:relative; z-index:2; text-align:center; display:flex; flex-direction:column; align-items:center; }
.clock-live { font-size:44px !important; line-height:1 !important; letter-spacing:-.03em !important; }
.clock-window { font-family:var(--font-display); font-size:13px; font-weight:700; margin-top:6px; color:var(--cyan); }
.clock-date { font-family:var(--font-main); font-size:11px; color:var(--grey-zone); margin-top:4px; }
.clock-copy h3 { font-size:30px !important; line-height:1.1; }
.coach-clock-copy p { font-size:14px; line-height:1.55; max-width:760px; }
.slot-status { border-radius:14px; padding:14px 16px; background:rgba(255,255,255,.025); }
.slot-status strong { font-size:18px !important; }
.next-slot-ribbon { border-radius:14px; padding:14px 16px; }
.next-slot-ribbon strong { font-size:18px !important; }

/* Energy Bank: tighter hero gauge and integrated pointer */
.energy-bank-card { border-radius:24px; padding:28px 24px 24px; }
.energy-bank-label { font-size:18px; font-weight:700; letter-spacing:-.01em; }
.energy-bank-explainer { font-size:12.5px; max-width:520px; }
.gauge-wrap { max-width:390px; margin-left:auto; margin-right:auto; }
.gauge-svg { filter:drop-shadow(0 14px 28px rgba(0,0,0,.28)); }
.gauge-segment { stroke-linecap:round; }
.gauge-pointer, .gauge-triangle { filter:drop-shadow(0 0 8px rgba(255,255,255,.35)); }
.energy-bank-score { font-family:var(--font-display) !important; font-weight:600; letter-spacing:-.04em; }
.energy-bank-status { border-radius:999px; padding:6px 13px; font-family:var(--font-main); font-weight:700; }

/* Unified metric cards */
.metrics-grid .metric-card, .metric-card { border-radius:18px; background:linear-gradient(180deg, rgba(255,255,255,.035), rgba(255,255,255,.015)), var(--panel); }
.metric-card { padding:18px 16px; min-height:150px; }
.metric-card .metric-label { font-family:var(--font-main); font-size:11px; font-weight:700; letter-spacing:.06em; }
.ring { width:74px; height:74px; border-radius:50%; position:relative; display:grid; place-items:center; margin:8px auto 7px;
  background:conic-gradient(var(--ring-color, var(--cyan)) calc(var(--pct, 0) * 1%), rgba(255,255,255,.07) 0);
}
.ring::before { content:''; position:absolute; inset:8px; border-radius:50%; background:#171717; box-shadow:inset 0 0 0 1px rgba(255,255,255,.05); }
.ring > * { position:relative; z-index:1; }
.metric-value { font-family:var(--font-display) !important; font-weight:600; letter-spacing:-.03em; }

/* Race readiness: more premium donut */
.readiness-card { border-radius:20px; background:linear-gradient(135deg, rgba(37,212,122,.08), rgba(255,255,255,.015)), var(--panel); }
.readiness-ring { width:126px; height:126px; border-radius:50%; display:grid; place-items:center; position:relative;
  box-shadow:0 18px 36px rgba(0,0,0,.22);
}
.readiness-ring::after { content:''; position:absolute; inset:12px; border-radius:50%; background:#161616; box-shadow:inset 0 0 0 1px rgba(255,255,255,.05); }
.readiness-ring-score { position:relative; z-index:2; font-size:32px !important; font-weight:600; }
.readiness-value { font-size:26px !important; line-height:1.1; font-weight:600; }

/* Softer global typography */
.page-title { font-weight:600; letter-spacing:-.03em; }
.section-title { font-weight:600; letter-spacing:-.025em; }
p, .subtitle, .prose-card p, .recommendation-box p, .training-tips-text { font-family:var(--font-main); }

@media (max-width: 760px) {
  .coach-clock-layout { grid-template-columns:1fr !important; }
  .coach-clock-visual { width:170px; height:170px; min-width:170px; margin:0 auto; }
  .clock-live { font-size:40px !important; }
  .top-bar .eyebrow { font-size:24px; }
  .logout-link { padding:8px 12px; font-size:12px; }
}


/* VISUAL POLISH v3.2 · gauge + clock refinement ----------------------- */
/* Clock uses the actual existing .clock-face structure. */
.coach-clock-card {
  grid-template-columns: 220px minmax(0,1fr);
  gap:30px; padding:26px; border-radius:24px;
  border-color:rgba(69,215,232,.18);
  background:radial-gradient(circle at 8% 5%, rgba(69,215,232,.13), transparent 34%),
             linear-gradient(145deg, rgba(255,255,255,.035), rgba(255,255,255,.012)), var(--panel);
}
.clock-face {
  width:190px; aspect-ratio:1; border-radius:50%; padding:0;
  background:conic-gradient(from -90deg, var(--cyan) 0 33%, rgba(255,255,255,.10) 33% 70%, var(--orange) 70% 100%);
  box-shadow:0 0 0 1px rgba(255,255,255,.05), 0 18px 40px rgba(0,0,0,.30), 0 0 34px rgba(69,215,232,.10);
}
.clock-face::after { inset:16px; background:radial-gradient(circle at 50% 20%, rgba(69,215,232,.07), transparent 40%), #111416; border-color:rgba(255,255,255,.07); }
.clock-face-inner { display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:156px; padding:16px; }
.clock-live { font-size:44px !important; font-weight:600; letter-spacing:-.045em !important; line-height:1 !important; }
.clock-window { margin-top:7px; font-family:var(--font-display); font-size:12px; font-weight:700; color:var(--white); }
.clock-zone { margin-top:5px; font-size:10px; letter-spacing:.08em; text-transform:none; color:var(--cyan); font-family:var(--font-main); font-weight:700; }
.clock-date { margin-top:5px; font-size:10px; color:var(--grey-zone); font-family:var(--font-main); }
.clock-copy h3 { font-size:29px !important; line-height:1.12; font-weight:600; letter-spacing:-.025em !important; }
.clock-copy > p { font-size:13px; line-height:1.6; color:#c5c8cd; }
.coach-call-kicker { font-size:20px !important; line-height:1; color:var(--red); }
.slot-status-grid { gap:12px; }
.slot-status { border-radius:15px; padding:14px 16px; }
.slot-status .slot-label { font-size:10px; letter-spacing:.06em; text-transform:none; font-family:var(--font-main); font-weight:700; }
.slot-status strong { font-size:18px !important; font-weight:700; }
.next-slot-ribbon { border-radius:15px; padding:14px 16px; }
.next-slot-ribbon span { text-transform:none; letter-spacing:.04em; font-family:var(--font-main); font-weight:700; }
.next-slot-ribbon strong { font-size:17px !important; font-weight:700; }
.today-activities { font-size:11px; color:#9ca2aa; }

/* Energy Bank marker is now a dot physically sitting on the arc. */
.gauge-marker-halo { fill:rgba(255,255,255,.18); stroke:rgba(255,255,255,.28); stroke-width:1.2; filter:drop-shadow(0 0 8px rgba(255,255,255,.32)); }
.gauge-marker { fill:#fff; stroke:#0b0b0b; stroke-width:1.5; filter:drop-shadow(0 2px 5px rgba(0,0,0,.55)); }
.gauge-segment { stroke-width:12.5; }
.gauge-score-wrap { bottom:5%; }
.energy-bank-score { font-size:46px !important; font-weight:600; letter-spacing:-.05em !important; }
.energy-ring-sub { font-family:var(--font-main); font-size:10px; letter-spacing:0; }
.zone-badge { font-family:var(--font-main) !important; font-weight:700; letter-spacing:.02em !important; }

/* All mini gauges share one consistent geometry. */
.gauge-card { min-height:156px; border-radius:18px; padding:17px 14px; background:linear-gradient(180deg, rgba(255,255,255,.035), rgba(255,255,255,.012)), var(--panel); }
.gauge-card-header { font-family:var(--font-main); font-weight:700; font-size:11px; letter-spacing:.04em; text-transform:none; }
.mini-ring { width:72px; height:72px; padding:7px; margin:10px auto 7px; box-shadow:0 0 20px -7px var(--ring-glow); }
.mini-ring-inner { background:#151515; box-shadow:inset 0 0 0 1px rgba(255,255,255,.05); }
.mini-ring-value { font-family:var(--font-display); font-size:17px; font-weight:600; letter-spacing:-.035em; }
.gauge-card-status { font-family:var(--font-main); font-size:10px; font-weight:700; letter-spacing:.04em; }
.trend-arrow { font-size:14px; }
.sleep-q { font-size:35px !important; font-weight:600; letter-spacing:-.04em !important; }
.sleep-q-label, .sleep-score-line { font-family:var(--font-main); }
.sleep-score-track { height:6px; border-radius:999px; }

/* Race Readiness donut: same ring language as the mini gauges. */
.readiness-strip { gap:14px; }
.readiness-panel { border-radius:18px; background:linear-gradient(180deg, rgba(255,255,255,.03), rgba(255,255,255,.01)), var(--panel); }
.readiness-panel.readiness-gauge-panel { grid-template-columns:126px 1fr; gap:20px; }
.readiness-ring { width:116px; aspect-ratio:1; border-radius:50%; background:conic-gradient(var(--readiness-color) calc(var(--readiness) * 1%), rgba(255,255,255,.08) 0); box-shadow:0 0 24px -8px var(--readiness-color), 0 18px 30px rgba(0,0,0,.22); }
.readiness-ring::after { inset:10px; background:#151515; }
.readiness-ring-score { font-size:30px !important; font-weight:600; letter-spacing:-.045em; }
.readiness-title { font-family:var(--font-main); font-weight:700; font-size:11px; letter-spacing:.04em; text-transform:none; }
.readiness-value { font-size:25px !important; font-weight:600; letter-spacing:-.03em !important; }
.readiness-reason { font-family:var(--font-main); line-height:1.45; }

/* Brand + buttons: soft, friendly, modern. */
.top-bar .eyebrow { font-family:var(--font-hand) !important; font-size:30px; font-weight:700; letter-spacing:.02em; text-transform:none; }
.performance-kicker { font-family:var(--font-hand) !important; font-size:21px; letter-spacing:.03em; text-transform:none; }
.btn, .checkin-btn { font-family:var(--font-display) !important; border-radius:12px; font-weight:600; letter-spacing:-.01em; }
.logout-link { min-height:38px; }

@media (max-width:760px) {
  .coach-clock-card { grid-template-columns:1fr; gap:20px; padding:20px; }
  .clock-face { width:172px; }
  .clock-face-inner { min-height:140px; }
  .clock-live { font-size:39px !important; }
  .clock-copy h3 { font-size:25px !important; }
  .slot-status-grid { grid-template-columns:1fr 1fr; }
  .next-slot-ribbon { align-items:flex-start; flex-direction:column; }
  .next-slot-ribbon strong { text-align:left; }
  .readiness-panel.readiness-gauge-panel { grid-template-columns:104px 1fr; }
  .readiness-ring { width:96px; }
}
@media (max-width:520px) {
  .slot-status-grid { grid-template-columns:1fr; }
  .dashboard-grid { grid-template-columns:repeat(2,minmax(0,1fr)); }
  .top-bar .eyebrow { font-size:26px; }
}

/* THE LAB · HOME TYPOGRAPHY v3.5 ---------------------------------------
   Home / pre-snapshot refinement. No backend behavior changes. */
:root {
  --home-card: rgba(19,21,24,.90);
...
body {
  background:
    radial-gradient(circle at 18% -8%, rgba(69,215,232,.10), transparent 28%),
    radial-gradient(circle at 82% 0%, rgba(255,59,79,.10), transparent 31%),
    radial-gradient(circle at 50% 120%, rgba(139,92,246,.07), transparent 34%),
    #08090a;
}

.wrap { max-width: 1320px; padding-top: 24px; }
...
.live-pill {
  ...
  border:1px solid rgba(69,215,232,.18);
  background:rgba(69,215,232,.055);
  color:#c8f7fb; font-size:11px; font-weight:700;
}
.live-pill::before {
  content:''; width:7px; height:7px; border-radius:50%;
  background:var(--green); box-shadow:0 0 12px rgba(37,212,122,.65);
}
.logout-link {
  min-height:38px; padding:0 15px !important; border-radius:999px !important;
  border:1px solid rgba(255,255,255,.14) !important;
  background:rgba(255,255,255,.035) !important;
  color:#f2f3f5 !important; font-family:var(--font-main) !important;
  font-size:12px !important; font-weight:700 !important;
  box-shadow:none !important;
}
.logout-link::before { content:'↗'; transform:rotate(90deg); opacity:.65; }
.logout-link:hover {
  background:rgba(255,59,79,.10) !important;
  border-color:rgba(255,59,79,.45) !important;
  color:#fff !important; transform:translateY(-1px);
}

.header-center.home-hero {
  position:relative; overflow:hidden;
  margin:0 auto 26px; padding:20px 24px 26px;
  border:1px solid rgba(255,255,255,.055);
  border-radius:28px;
  background:linear-gradient(180deg, rgba(255,255,255,.018), rgba(255,255,255,.008));
}
.header-center.home-hero::before {
  content:''; position:absolute; width:420px; height:220px; border-radius:50%;
  left:50%; top:-165px; transform:translateX(-50%);
  background:rgba(69,215,232,.12); filter:blur(55px); pointer-events:none;
}
.home-logo {
  position:relative; z-index:1;
  width:210px; margin:2px auto 12px;
  filter:drop-shadow(0 12px 24px rgba(0,0,0,.42));
}
h1.page-title {
  position:relative; z-index:1;
  margin:0; color:#f7f7f8 !important;
  font-family:var(--font-display) !important;
  font-size:clamp(42px,5vw,64px) !important;
  line-height:.98; font-weight:600 !important;
...
.hero-script {
  ...
  color:var(--cyan); margin-top:13px; font-weight:600;
  text-transform:uppercase; letter-spacing:.14em;
}
.header-center .subtitle {
  position:relative; z-index:1;
  max-width:680px; margin:10px auto 0;
  color:#aeb4bd; font-size:12.5px; line-height:1.55;
}
.performance-kicker {
  position:relative; z-index:1; display:inline-flex; align-items:center; gap:7px;
  margin:13px auto 0 !important; padding:6px 10px; border-radius:999px;
  border:1px solid rgba(255,255,255,.08);
  background:rgba(255,255,255,.025);
  color:#d3d7dd !important; font-family:var(--font-hand) !important;
  font-size:19px !important; line-height:1 !important; letter-spacing:.03em !important;
  text-transform:none !important;
}
.performance-kicker::before {
  content:''; width:6px; height:6px; border-radius:50%; background:var(--cyan);
  box-shadow:0 0 10px rgba(69,215,232,.55);
}
...
.control-card {
  ...
  background:linear-gradient(180deg, rgba(255,255,255,.028), rgba(255,255,255,.012)), var(--home-card) !important;
  box-shadow:0 16px 38px rgba(0,0,0,.24) !important;
  transition:transform .18s ease, border-color .18s ease, box-shadow .18s ease;
}
.control-card::before {
  content:''; position:absolute; left:0; top:0; right:0; height:2px; opacity:.9;
  background:linear-gradient(90deg, transparent, var(--card-accent, var(--cyan)), transparent);
}
.control-card:hover {
  transform:translateY(-2px);
  border-color:rgba(255,255,255,.16) !important;
  box-shadow:0 22px 44px rgba(0,0,0,.31) !important;
}
.chat-section { --card-accent:var(--cyan); }
.checkin-card { --card-accent:var(--purple); }
.snapshot-card { --card-accent:var(--orange); }
.control-card .section-title {
  display:flex; align-items:center; gap:10px;
  margin:0 0 8px; color:#f4f5f7 !important;
  font-family:var(--font-display) !important;
  font-size:26px !important; line-height:1.12;
  font-weight:600; letter-spacing:-.035em !important;
  text-transform:none !important;
}
.control-icon {
  width:34px; height:34px; margin:0 !important;
  display:inline-grid; place-items:center; flex:0 0 34px;
  border-radius:11px; font-size:16px !important;
  background:rgba(255,255,255,.045); border:1px solid rgba(255,255,255,.075);
}
.control-card .subtitle {
  color:#c3c8cf; font-size:15px; line-height:1.6; margin:0 0 20px;
  min-height:39px;
}

.control-chat-form { grid-template-columns:1fr auto; gap:10px; align-items:stretch; }
.control-chat-form .chat-input {
  height:54px !important; padding:0 15px !important;
  border:1px solid rgba(255,255,255,.12) !important; border-radius:14px !important;
  background:rgba(5,6,7,.55) !important; color:#f5f5f5 !important;
  font-family:var(--font-main) !important; font-size:15px !important;
  outline:none; transition:.18s ease;
}
.control-chat-form .chat-input:focus {
  border-color:rgba(69,215,232,.55) !important;
  box-shadow:0 0 0 3px rgba(69,215,232,.08);
}
.control-chat-form .btn {
  width:auto !important; min-width:100px; height:54px; margin:0 !important; padding:0 19px !important;
  border-radius:14px !important; border:1px solid rgba(69,215,232,.20) !important;
  background:linear-gradient(135deg, rgba(69,215,232,.95), #28b7ca) !important;
  color:#071013 !important; font-size:13px !important; font-weight:700 !important;
  box-shadow:0 10px 25px rgba(69,215,232,.13);
}
.control-chat-form .btn:hover { filter:brightness(1.05); transform:translateY(-1px); }
.notes-list {
  border-top:1px solid rgba(255,255,255,.07); padding-top:12px;
  scrollbar-width:thin; scrollbar-color:rgba(255,255,255,.14) transparent;
}
.note-line { font-size:11px; color:#aab0b9; line-height:1.45; }

.checkin-row { margin-top:4px; }
.checkin-card .checkin-buttons { gap:10px 8px !important; }
.checkin-card .checkin-btn {
  width:44px !important; height:44px !important;
  border:1px solid rgba(255,255,255,.14) !important;
  border-radius:50% !important; background:rgba(255,255,255,.025) !important;
  color:#eef0f3 !important; font-family:var(--font-display) !important;
  font-size:13px !important; font-weight:600 !important;
}
.checkin-card .checkin-btn:hover {
  background:rgba(139,92,246,.18) !important;
  border-color:rgba(139,92,246,.65) !important;
  color:#fff !important; box-shadow:0 0 0 4px rgba(139,92,246,.08) !important;
}
.checkin-scale {
  margin-top:18px !important; padding-top:14px !important;
  font-family:var(--font-main); font-size:10px !important;
}
.checkin-scale span { padding:3px 7px; border-radius:999px; background:rgba(255,255,255,.025); }

.snapshot-status-box {
  border:1px solid rgba(255,159,28,.18) !important;
  border-radius:18px !important; padding:18px !important;
  background:
    radial-gradient(circle at 50% -10%, rgba(255,159,28,.11), transparent 44%),
    rgba(7,8,9,.34) !important;
  justify-content:flex-start !important;
}
.status-pulse {
  width:11px !important; height:11px !important; margin:2px auto 8px !important;
  border:none !important; opacity:1 !important;
  background:var(--green); box-shadow:0 0 0 5px rgba(37,212,122,.08),0 0 18px rgba(37,212,122,.35);
}
.snapshot-card.is-loading .status-pulse {
  animation:homePulse 1.2s ease-in-out infinite !important; background:var(--orange);
}
@keyframes homePulse { 50% { transform:scale(.72); opacity:.55; } }
.snapshot-status-title {
  font-family:var(--font-display) !important;
  font-size:27px !important; line-height:1; color:#f4f5f7 !important;
  font-weight:600; letter-spacing:-.04em !important; text-transform:none !important;
}
.snapshot-status-copy { max-width:290px; margin:8px auto 13px; font-size:11.5px !important; color:#aeb4bd !important; }
.coach-clock-preview {
  width:100%; display:grid !important; grid-template-columns:auto auto 1fr !important;
  align-items:center; gap:10px !important;
  padding:11px 12px !important; margin:0 0 12px !important;
  border:1px solid rgba(69,215,232,.16) !important; border-radius:14px !important;
  background:rgba(69,215,232,.045) !important; text-align:left;
}
.coach-clock-preview .clock-mini-label {
  font-family:var(--font-hand) !important; font-size:18px !important; line-height:1;
  color:#bff7fb !important; text-transform:none !important; letter-spacing:.02em !important;
}
.coach-clock-preview strong {
  font-family:var(--font-display) !important; font-size:25px !important;
  letter-spacing:-.045em !important; color:#fff;
}
.coach-clock-preview small {
  justify-self:end; font-family:var(--font-main); font-size:9px !important; line-height:1.25;
  color:#aeb4bd !important; text-align:right; text-transform:none !important; letter-spacing:.02em !important;
}
.snapshot-card #snapshot-btn {
  height:54px; border-radius:14px !important;
  border:1px solid rgba(255,159,28,.18) !important;
  background:linear-gradient(135deg, #ffb12d, var(--orange)) !important;
  color:#171009 !important; font-family:var(--font-display) !important;
  font-weight:700 !important; font-size:14px !important;
  box-shadow:0 12px 28px rgba(255,159,28,.13);
}
.snapshot-card #snapshot-btn:hover { filter:brightness(1.06); transform:translateY(-1px); }
.loading-track { height:7px !important; border:none !important; background:rgba(255,255,255,.07) !important; }
.loading-fill { background:linear-gradient(90deg,var(--orange),var(--cyan)) !important; }
.loading-label { color:#aeb4bd !important; }


/* v3.5 readability and check-in scale */
.control-card .snapshot-status-copy { font-size:14px !important; line-height:1.55 !important; color:#c3c8cf !important; }
.snapshot-status-title { font-family:var(--font-display) !important; font-size:30px !important; }
.coach-clock-preview { font-family:var(--font-main) !important; }
.clock-mini-label { font-size:11px !important; letter-spacing:.10em !important; }
.coach-clock-preview strong { font-family:var(--font-display) !important; font-size:27px !important; }
.coach-clock-preview small { font-size:11px !important; line-height:1.25 !important; }
.checkin-card .checkin-btn { font-family:var(--font-display) !important; font-size:15px !important; }
.checkin-scale { display:grid !important; grid-template-columns:1fr 1fr 1fr; align-items:center; margin-top:14px !important; padding-top:12px; border-top:1px solid rgba(255,255,255,.07); font-family:var(--font-main); font-size:11px !important; color:#8f959e; }
.checkin-scale span:nth-child(1){ text-align:left; color:#ff6b7b; }
.checkin-scale span:nth-child(2){ text-align:center; color:#b4bac3; }
.checkin-scale span:nth-child(3){ text-align:right; color:#54dfa0; }
.note-line { font-size:13px !important; }
.live-pill, .logout-link { font-family:var(--font-main) !important; }
@media (max-width:980px) {
  .control-deck { grid-template-columns:1fr 1fr; }
  .snapshot-card { grid-column:1 / -1; min-height:auto; }
  .snapshot-status-box { max-width:none; }
}
@media (max-width:650px) {
  .wrap { padding:14px 11px 44px; }
  .top-bar { margin-bottom:12px; }
  .top-bar .eyebrow { font-size:26px !important; }
  .live-pill { display:none; }
  .header-center.home-hero { padding:18px 13px 20px; border-radius:22px; margin-bottom:16px; }
  .home-logo { width:175px !important; }
  h1.page-title { font-size:42px !important; }
  .hero-script { font-size:23px; }
  .header-center .subtitle { font-size:11px; max-width:330px; }
  .performance-kicker { font-size:17px !important; }
  .control-deck { grid-template-columns:1fr; gap:12px; }
  .control-card { min-height:auto; padding:17px 15px; border-radius:18px !important; }
  .control-card .subtitle { min-height:0; }
  .control-chat-form { grid-template-columns:1fr; }
  .control-chat-form .btn { width:100% !important; }
  .coach-clock-preview { grid-template-columns:1fr auto !important; }
  .coach-clock-preview small { grid-column:1 / -1; justify-self:start; text-align:left; }
  .checkin-card .checkin-btn { width:41px !important; height:41px !important; }
}
@media (max-width:390px) {
  .top-bar .eyebrow { font-size:23px !important; }
  .logout-link { padding:0 12px !important; }
  h1.page-title { font-size:37px !important; }
  .control-card .section-title { font-size:21px !important; }
}



/* v3.6 FONT FIX — explicit, robust typography + discreet build mark */
:root {
  --font-display: "Parkinsans", "Trebuchet MS", Arial, sans-serif;
  --font-main: "Manrope", "Trebuchet MS", Arial, sans-serif;
  --font-hand: "Amatic SC", "Segoe Print", "Bradley Hand", cursive;
}
body, input, button, textarea, select { font-family:var(--font-main) !important; }
.display, h1, h2, h3, .section-title, .page-title, .snapshot-status-title,
.energy-bank-score, .mini-ring-value, .readiness-ring-score, .session-title,
.coach-clock-preview strong { font-family:var(--font-display) !important; }
.top-bar .eyebrow {
  font-family:var(--font-hand) !important;
  font-weight:400 !important;
...
.build-mark {
  ...
  font-weight:500;
  letter-spacing:.04em;
  color:rgba(190,196,205,.38);
  pointer-events:none;
  user-select:none;
}
@media (max-width:650px) {
  .top-bar .eyebrow { font-size:34px !important; }
  .control-card .subtitle, .snapshot-status-copy { font-size:14px !important; }
  .build-mark { right:8px; bottom:6px; font-size:8px; }
}


/* THE LAB HOME REBUILD v3.7 — isolated namespace, no legacy class dependency */
.lab-home-shell { max-width: 1480px; margin: 0 auto; padding: 26px 22px 72px; position: relative; }
.lab-home-topbar { display:flex; align-items:center; justify-content:space-between; gap:18px; margin:0 4px 26px; }
.lab-home-brand { font-family:'Amatic SC','Segoe Print',cursive !important; font-size:42px; line-height:1; font-weight:400 !important; letter-spacing:.02em; color:#f7f7f7; }
.lab-home-actions { display:flex; align-items:center; gap:10px; }
.lab-home-live,.lab-home-logout { font-family:'Manrope',Arial,sans-serif !important; font-size:13px; font-weight:700; border-radius:999px; padding:10px 14px; text-decoration:none; }
.lab-home-live { color:#8feaff; border:1px solid rgba(69,215,232,.28); background:rgba(69,215,232,.07); }
.lab-home-logout { color:#f5f5f5; border:1px solid rgba(255,255,255,.17); background:rgba(255,255,255,.04); transition:.18s ease; }
.lab-home-logout:hover { border-color:rgba(255,59,79,.65); background:rgba(255,59,79,.10); transform:translateY(-1px); }
.lab-home-hero { text-align:center; padding:28px 24px 30px; border:1px solid rgba(255,255,255,.08); border-radius:30px; background:radial-gradient(circle at 18% 10%,rgba(69,215,232,.07),transparent 34%),radial-gradient(circle at 82% 4%,rgba(255,59,79,.09),transparent 35%),rgba(15,16,17,.78); box-shadow:0 20px 50px rgba(0,0,0,.22); }
.lab-home-logo { width:230px; max-width:48vw; height:auto; display:block; margin:0 auto 16px; }
.lab-home-title { font-family:'Parkinsans','Trebuchet MS',Arial,sans-serif !important; font-size:clamp(46px,5.2vw,76px); line-height:1.02; font-weight:600 !important; letter-spacing:-.045em; margin:0; color:#f7f7f7; }
.lab-home-kicker { font-family:'Parkinsans','Trebuchet MS',Arial,sans-serif !important; color:#45d7e8; font-size:13px; font-weight:700; letter-spacing:.22em; margin-top:14px; }
.lab-home-subtitle { font-family:'Manrope',Arial,sans-serif !important; color:#aeb5bd; font-size:15px; line-height:1.55; margin:10px 0 0; }
.lab-home-deck { display:grid; grid-template-columns:1.08fr .96fr .96fr; gap:18px; margin-top:22px; align-items:stretch; }
.lab-home-card { min-height:330px; padding:24px; border-radius:24px; border:1px solid rgba(255,255,255,.11); background:linear-gradient(180deg,rgba(28,30,32,.98),rgba(20,22,24,.98)); box-shadow:0 16px 40px rgba(0,0,0,.24); }
.lab-home-card-title { display:flex; align-items:center; gap:11px; font-family:'Parkinsans','Trebuchet MS',Arial,sans-serif !important; font-size:26px; line-height:1.15; font-weight:600 !important; letter-spacing:-.03em; color:#f7f7f7; margin:0 0 12px; }
.lab-home-icon { width:38px; height:38px; display:inline-grid; place-items:center; flex:0 0 auto; border-radius:12px; background:rgba(255,255,255,.055); border:1px solid rgba(255,255,255,.08); font-size:18px; }
.lab-home-card-copy { font-family:'Manrope',Arial,sans-serif !important; color:#b8bec6; font-size:16px; line-height:1.65; margin:0 0 20px; }
.lab-home-chat-form { display:grid; grid-template-columns:minmax(0,1fr) 118px; gap:12px; margin-top:10px; }
.lab-home-chat-input { min-width:0; width:100%; height:58px; padding:0 17px; border-radius:16px; border:1px solid rgba(255,255,255,.12); background:#0f1112; color:#fff; font-family:'Manrope',Arial,sans-serif !important; font-size:15px; outline:none; }
.lab-home-chat-input:focus { border-color:rgba(69,215,232,.58); box-shadow:0 0 0 3px rgba(69,215,232,.08); }
.lab-home-send,.lab-home-generate { border:0; cursor:pointer; font-family:'Parkinsans','Trebuchet MS',Arial,sans-serif !important; font-weight:700; border-radius:16px; transition:.18s ease; }
.lab-home-send { background:linear-gradient(135deg,#52d8e7,#2fb9d0); color:#071012; font-size:15px; }
.lab-home-send:hover,.lab-home-generate:hover { transform:translateY(-1px); filter:brightness(1.05); }
.lab-home-checkin-row { display:block; }
.lab-home-checkin-buttons { display:grid; grid-template-columns:repeat(5,1fr); gap:11px; width:100%; }
.lab-home-checkin-btn { aspect-ratio:1; width:100%; max-width:58px; justify-self:center; border-radius:50%; border:1px solid rgba(255,255,255,.16); background:rgba(255,255,255,.015); color:#f7f7f7; font-family:'Parkinsans','Trebuchet MS',Arial,sans-serif !important; font-size:16px; font-weight:600; cursor:pointer; transition:.16s ease; }
.lab-home-checkin-btn:hover { border-color:#45d7e8; background:rgba(69,215,232,.09); transform:translateY(-2px); }
.lab-home-checkin-scale { display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; padding-top:18px; margin-top:18px; border-top:1px solid rgba(255,255,255,.08); font-family:'Manrope',Arial,sans-serif !important; font-size:12px; font-weight:700; }
.lab-home-checkin-scale span:nth-child(1){color:#ff6678;text-align:left}.lab-home-checkin-scale span:nth-child(2){color:#aeb5bd;text-align:center}.lab-home-checkin-scale span:nth-child(3){color:#33dc8a;text-align:right}
.lab-home-status-box { border:1px solid rgba(255,159,28,.30); border-radius:20px; padding:20px; background:radial-gradient(circle at 50% 0,rgba(255,159,28,.08),transparent 46%),rgba(13,15,16,.72); text-align:center; }
.lab-home-status-pulse { width:12px;height:12px;border-radius:50%;background:#25d47a;box-shadow:0 0 0 6px rgba(37,212,122,.10),0 0 18px rgba(37,212,122,.55);margin:0 auto 11px; }
.lab-home-status-title { font-family:'Parkinsans','Trebuchet MS',Arial,sans-serif !important; font-size:29px; font-weight:600; letter-spacing:-.03em; color:#f7f7f7; }
.lab-home-status-copy { font-family:'Manrope',Arial,sans-serif !important; font-size:14px; line-height:1.55; color:#b7bdc5; max-width:330px; margin:7px auto 16px; }
.lab-home-clock { display:grid; grid-template-columns:auto auto 1fr; align-items:center; gap:11px; min-height:70px; padding:12px 14px; border-radius:16px; border:1px solid rgba(69,215,232,.23); background:rgba(69,215,232,.055); text-align:left; }
.lab-home-clock-label { font-family:'Parkinsans','Trebuchet MS',Arial,sans-serif !important; color:#7be6f1; font-size:11px; font-weight:700; letter-spacing:.13em; white-space:nowrap; }
.lab-home-clock strong { font-family:'Parkinsans','Trebuchet MS',Arial,sans-serif !important; font-size:28px; line-height:1; color:#fff; }
.lab-home-clock small { font-family:'Manrope',Arial,sans-serif !important; font-size:12px; line-height:1.25; color:#aeb7c0; text-align:right; }
.lab-home-generate { width:100%; min-height:58px; margin-top:14px; background:linear-gradient(135deg,#ffb62f,#ff9718); color:#111; font-size:16px; }
.lab-home-build { position:fixed; right:14px; bottom:10px; z-index:20; font-family:'Manrope',Arial,sans-serif !important; font-size:9px; line-height:1; letter-spacing:.08em; color:rgba(255,255,255,.26); pointer-events:none; }
@media(max-width:1000px){.lab-home-deck{grid-template-columns:1fr}.lab-home-card{min-height:0}.lab-home-shell{padding-left:16px;padding-right:16px}}
@media(max-width:620px){.lab-home-topbar{align-items:flex-start}.lab-home-brand{font-size:34px}.lab-home-actions{gap:6px}.lab-home-live{display:none}.lab-home-logout{font-size:12px;padding:9px 12px}.lab-home-hero{padding:22px 16px 24px;border-radius:24px}.lab-home-logo{width:190px}.lab-home-title{font-size:44px}.lab-home-kicker{font-size:11px;letter-spacing:.15em}.lab-home-subtitle{font-size:13px}.lab-home-card{padding:20px}.lab-home-card-title{font-size:23px}.lab-home-card-copy{font-size:15px}.lab-home-chat-form{grid-template-columns:1fr}.lab-home-send{height:52px}.lab-home-checkin-buttons{gap:8px}.lab-home-clock{grid-template-columns:1fr auto}.lab-home-clock-label{grid-column:1/-1}.lab-home-clock small{font-size:11px}}
"""
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Amatic+SC:wght@400;700&family=Manrope:wght@400;500;600;700&family=Parkinsans:wght@400;500;600;700&display=swap" rel="stylesheet">
...
<body>
  <div class="wrap lab-home-shell">
    <div class="lab-home-topbar">
      <span class="lab-home-brand">The Gluten Free Cyclist</span>
      <div class="lab-home-actions">
        <span class="lab-home-live">Live · Europe/Rome</span>
        <a class="lab-home-logout" href="{{ url_for('logout') }}">Log out</a>
      </div>
    </div>
    <div class="lab-home-hero">
      <img class="lab-home-logo" src="data:image/png;base64,{{ logo }}" alt="The Gluten Free Cyclist">
      <h1 class="lab-home-title">Health Snapshot</h1>
      <div class="lab-home-kicker">YOUR DAILY TRAINING DESK</div>
      <p class="lab-home-subtitle">Last {{ days }} days for recovery &middot; {{ season_days }} days for training context &middot; Intervals.icu + AI coach</p>
    </div>

    <div class="lab-home-deck no-print">
      <section class="lab-home-card lab-home-chat">
        <h2 class="lab-home-card-title"><span class="lab-home-icon">&#128172;</span>Coach Chat</h2>
        <p class="lab-home-card-copy">Tell the coach anything that should change today's interpretation or the next sessions.</p>
        <form method="post" action="{{ url_for('ask') }}" id="chat-form" class="lab-home-chat-form">
          <input type="text" class="lab-home-chat-input" name="question" placeholder="e.g. Legs feel unusually heavy today" required>
          <button type="submit" class="lab-home-send" id="chat-btn">Send</button>
        </form>
        {% if chat_error %}<div class="error-panel" style="margin-top:14px;">{{ chat_error }}</div>{% endif %}
        {% if chat_answer %}<p class="chat-noted display">{{ chat_answer }}</p>{% endif %}
        {% if notes %}
        <div class="notes-list">
          <p class="stat-label" style="margin-bottom:8px;">Remembered so far</p>
          {% for n in notes|reverse %}<p class="note-line"><span class="note-date">{{ n.date }}</span> {{ n.text }}</p>{% endfor %}
        </div>
        {% endif %}
      </section>

      <section class="lab-home-card lab-home-checkin">
        <h2 class="lab-home-card-title"><span class="lab-home-icon">&#128203;</span>Daily Check-In</h2>
        <p class="lab-home-card-copy">How do you feel right now? Your own signal sits beside the wearable data.</p>
        <div class="lab-home-checkin-row">
          {% if latest_feeling %}
          ...
          {% endif %}
          <form method="post" action="{{ url_for('log_feeling') }}" class="lab-home-checkin-buttons">
            {% for n in range(1, 11) %}<button type="submit" name="feeling" value="{{ n }}" class="lab-home-checkin-btn">{{ n }}</button>{% endfor %}
          </form>
        </div>
        <div class="lab-home-checkin-scale"><span>1 · Terrible</span><span>5–6 · Neutral</span><span>10 · Amazing</span></div>
      </section>

      <section class="lab-home-card lab-home-snapshot" id="snapshot-card">
        <h2 class="lab-home-card-title"><span class="lab-home-icon">&#9889;</span>Snapshot Status</h2>
        <div class="lab-home-status-box">
          <div class="lab-home-status-pulse" aria-hidden="true"></div>
          <div class="lab-home-status-title" id="snapshot-status-title">Ready</div>
          <div class="lab-home-status-copy" id="snapshot-status-copy">Pull fresh Intervals.icu data, then let the coach decide what comes next.</div>
          <div class="lab-home-clock">
            <span class="lab-home-clock-label">COACH CLOCK</span>
            <strong>{{ clock_preview.time }}</strong>
            <small>{{ clock_preview.phase }}</small>
          </div>
          <form method="post" action="{{ url_for('analyze') }}" id="snapshot-form">
            <button type="submit" class="lab-home-generate" id="snapshot-btn">Generate Snapshot</button>
    ...
    <div class="lab-home-build">{{ app_version }}</div>
    ...
    <div class="section dashboard-section">
      ...
      <div class="coach-clock-card">
        <div class="clock-face" style="--clock-pct:{{ data.coach_clock.day_progress }};">
          <div class="clock-face-inner">
            <div class="clock-live">{{ data.coach_clock.time }}</div>
              <div class="clock-window">{{ data.coach_clock.window_label }}</div>
            <div class="clock-zone">{{ data.coach_clock.phase }}</div>
            <div class="clock-date">{{ data.coach_clock.date_label }}</div>
          </div>
        </div>
        <div class="clock-copy">
          <div class="coach-call-kicker">COACH CLOCK · EUROPE/ROME · HOME TYPOGRAPHY v3.5</div>
          <h3>{{ data.coach_clock.headline }}</h3>
          <p>{{ data.coach_clock.explanation }}</p>
          <div class="slot-status-grid">
            <div class="slot-status {{ data.coach_clock.am_status_class }}">
              <div class="slot-label">Morning · before 12</div>
              <strong>{{ data.coach_clock.am_status }}</strong>
            </div>
            <div class="slot-status {{ data.coach_clock.pm_status_class }}">
              <div class="slot-label">Evening · after 17</div>
              <strong>{{ data.coach_clock.pm_status }}</strong>
            </div>
          </div>
          <div class="next-slot-ribbon"><span>Next decision slot</span><strong>{{ data.coach_clock.next_slot_label }}</strong></div>
          {% if data.coach_clock.today_activities_text %}<div class="today-activities">Logged today: {{ data.coach_clock.today_activities_text }}</div>{% endif %}
        </div>
      </div>
      ...
      <div class="gauge-wrap">
        <svg class="gauge-svg" viewBox="0 0 200 112" aria-label="Energy Bank gauge">
          ...
          <circle class="gauge-marker-halo" cx="{{ data.marker_x }}" cy="{{ data.marker_y }}" r="7.2" aria-hidden="true"/>
          <circle class="gauge-marker" cx="{{ data.marker_x }}" cy="{{ data.marker_y }}" r="4.4" aria-label="Current Energy Bank position"/>
        </svg>
      ...
      <div class="readiness-strip">
        <div class="readiness-panel readiness-gauge-panel">
          <div class="readiness-ring" style="--readiness:{{ data.race_readiness_score }}; --readiness-color:var(--{{ data.race_readiness_css_color }});">
            <div class="readiness-ring-inner"><div class="readiness-ring-score">{{ data.race_readiness_score }}</div><div class="readiness-ring-sub">/ 100</div></div>
          </div>
          <div>
    ...
    <div class="coach-call-box">
      <div class="coach-call-kicker">COACH CALL</div>
      <div class="coach-call-title">What I would do from here</div>
      <p>{{ data.recommendation }}</p>
    </div>

    <div class="next-sessions-section">
      <div class="next-sessions-head">
        <h2 class="section-title display">Next 3 Sessions</h2>
        <p>Built from the snapshot time, today's activities and the training already completed this week.</p>
      </div>
      <div class="week-memory-bar">
        <div class="week-memory-label">This week</div>
        <div class="week-memory-items">
          {% if data.week_memory and data.week_memory.activities %}
            {% for a in data.week_memory.activities %}
            <span class="week-memory-chip {{ 'hard' if a.hard else 'easy' }}"><strong>{{ a.day_short }}</strong> {{ a.family_short }}</span>
            {% endfor %}
          {% else %}
            <span class="week-memory-empty">No completed sessions found yet this week.</span>
          {% endif %}
        </div>
        {% if data.week_memory and data.week_memory.hard_summary %}<div class="week-memory-note">Quality already completed: {{ data.week_memory.hard_summary }}</div>{% endif %}
      </div>
      <div class="next-sessions-grid">
        {% for s in data.next_sessions %}
        <article class="session-card intensity-{{ s.intensity_class }}">
          <div class="session-slot">{{ s.slot }}</div>
          <div class="session-title">{{ s.title }}</div>
          <div class="session-meta"><span class="session-pill">{{ s.duration }}</span><span class="session-pill">{{ s.intensity }}</span></div>
          <div class="session-main">{{ s.main_set }}</div>
          <div class="session-why"><strong>Why:</strong> {{ s.why }}</div>
        </article>
        {% endfor %}
      </div>
    </div>
    ...
    (function () {
      ...
      if (form) {
        form.addEventListener('submit', function () {
          ...
          if (snapshotCard) snapshotCard.classList.add('is-loading');
  ...
  <div class="build-mark">{{ app_version }}</div>
</body>
</html>
"""
...
def home():
    ...
    return render_template_string(
        ...
        app_version=APP_VERSION, clock_preview=get_clock_preview(),
    )
